...
OFFICIAL_ALIAS_FILE = OUTPUT_DIR / "mamba_uav_dynamic_profiles_official.csv"
...
try:
    from mamba_ssm import Mamba as OfficialMambaBlock
except Exception:
    OfficialMambaBlock = None
...
class OfficialMambaSequenceBlock(nn.Module):
    """Optional official mamba-ssm backend with the same residual interface."""

    def __init__(self, dim: int, dropout: float) -> None:
        super().__init__()
        if OfficialMambaBlock is None:
            raise RuntimeError("mamba_ssm is not installed.")
        self.norm = nn.LayerNorm(dim)
        self.mamba = OfficialMambaBlock(d_model=dim, d_state=16, d_conv=4, expand=2)
        self.dropout = nn.Dropout(dropout)
...
class TorchMambaProfileModel(nn.Module):
    def __init__(
        ...
        super().__init__()
        if backend == "auto":
            backend = "official_mamba" if OfficialMambaBlock is not None else "selective_ssm"
        if backend not in {"official_mamba", "selective_ssm"}:
        ...
        block_cls = OfficialMambaSequenceBlock if backend == "official_mamba" else SelectiveStateSpaceBlock
        self.blocks = nn.ModuleList([block_cls(model_dim, dropout=dropout) for _ in range(depth)])
...
def write_dynamic_profiles(metadata: list[WindowMeta], predictions: np.ndarray) -> None:
    ...
    OFFICIAL_ALIAS_FILE.write_text(OUTPUT_FILE.read_text(encoding="utf-8"), encoding="utf-8")
...
def main() -> None:
    ...
    print(f"mamba_backend={model.backend}")
    ...
    metrics = {
        ...
        "mamba_backend": model.backend,
        "official_mamba_available": OfficialMambaBlock is not None,
        "device": str(device),
        ...
        "official_alias_file": str(OFFICIAL_ALIAS_FILE),
        "model_file": str(MODEL_FILE),
        "note": "Uses official mamba-ssm when installed; otherwise falls back to a compact selective state-space scan with the same CSV interface.",
    }
    ...
    print(f"Generated alias: {OFFICIAL_ALIAS_FILE}")
