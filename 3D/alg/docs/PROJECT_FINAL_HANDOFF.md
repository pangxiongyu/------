...
1. 使用 Mamba / Torch Mamba 风格动态个体画像作为无人机能力输入。
2. 使用 GPR 三维动态气象地图刻画空间、时间和高度上的风险/代价。
...
| Mamba 画像接入 | 已完成 | 支持 Torch Mamba 风格画像，并提供官方 mamba-ssm 可选后端接口 |
| GPR 气象建模 | 已完成 | 已生成 `weather_cost_map_gpr_sample_24h.csv` 与完整 GPR 气象地图 |
| 基线规划器 | 已完成 | one_shot、sequential、weather_grid、marl_greedy 均可运行 |
| 二维/三维气象路径规划 | 已完成 | 支持 weather-grid 与 weather-3D 路径动作，并可读取 GPR 气象代价 |
...
| 报告导出 | 已完成 | 已生成阶段报告、交接文档、复现实验命令清单和 waypoint 可视化导出 |
...
### 4.0 GPR + Mamba + MAPPO 对齐版本

当前已经新增对齐项目书算法口径的 benchmark：

```text
outputs/gpr_mamba_mappo_benchmark/comparison.md
...
```text
GPR weather map + Mamba dynamic profiles + MAPPO + Robust MPC
```

其中 MAPPO 完成全部 5 个任务，使用 3 条 weather-3D 路径和 2 条 weather-grid 路径，MPC 约束违反次数为 0。`planned_routes_waypoints.csv` 可直接给 Qt/Web 三维可视化使用。
