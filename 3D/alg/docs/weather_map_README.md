...
本目录包含 IDW 原型版和 GPR 对齐项目书版本的三维动态气象代价地图，用于 Web/QT 可视化和路径规划联调。
...
- `weather_cost_map_gpr.csv`
  - GPR 生成的完整三维动态气象代价地图。
  - 当前路径规划默认推荐使用。

- `weather_cost_map_gpr_sample_24h.csv`
  - GPR 生成的前 24 小时样本数据。
  - 当前 `configs/default.yaml` 默认读取该文件。

- `gpr_weather_metrics.json`
  - GPR 预测不确定性和建模元信息。
...
由于现有 `output_data` 没有精确经纬度字段，本版本采用地区代表点作为稀疏观测点。历史版本通过 IDW（反距离加权）插值生成网格化代价地图；当前对齐项目书版本通过 GPR（高斯过程回归）生成网格化代价地图。
