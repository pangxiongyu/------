...
function generateheightmap(width, depth, resolution, scale = 0.04) {
...
function getheightat(heights, resolution, ix, iz) {
...
export { getheightat };
function getterraincolor(height, maxheight = 15) {
  const t = height / maxheight;
  if (t < 0.15) return [0.18, 0.55, 0.2]; // 深绿色（山谷和草地）
  if (t < 0.35) return [0.25, 0.6, 0.18]; // 绿色（草地）
  if (t < 0.55) return [0.35, 0.5, 0.15]; // 橄榄绿（山坡）
  if (t < 0.70) return [0.45, 0.4, 0.2]; // 棕色（岩石区）
  if (t < 0.85) return [0.55, 0.5, 0.4]; // 灰棕色
  return [0.75, 0.75, 0.8]; // 灰白色（山顶）
}
