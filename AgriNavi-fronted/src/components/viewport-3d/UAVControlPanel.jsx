...
export default function UAVControlPanel() {
  return <div className="bg-white/90 backdrop-blur-md rounded-2xl shadow-xl border border-white/40 p-6 h-full flex flex-col">
      ...
      <div className="space-y-2 flex-1 overflow-y-auto">
        {dronefleet.map(drone => <UAVCard key={drone.id} drone={drone} />)}
      </div>
      ...
      <div className="mt-4 pt-4 border-t border-gray-100">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="font-bold text-agri-500">
            {dronefleet.filter(d => d.status === 'active').length} 飞行中
          </span>
        </div>
        <div className="flex items-center justify-between text-sm mt-1">
          <span className="text-gray-500">平均电量</span>
          <span className="font-bold text-dark">
            {Math.round(dronefleet.reduce((s, d) => s + d.battery, 0) / dronefleet.length)}%
          </span>
        </div>
      </div>
    </div>;
