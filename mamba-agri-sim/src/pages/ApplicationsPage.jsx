...
import { AGRICULTURE_APPS } from '../data/agricultureApps'
...
export default function ApplicationsPage() {
  ...
  const app = AGRICULTURE_APPS[selectedApp]
    ...
    <PageContainer>
      <div className="max-w-7xl mx-auto px-6 py-20">
        <SectionTitle
          badge="应用矩阵"
          title="智慧农业应用"
          subtitle="基于无人机遥感与 AI 分析的全方位智慧农业应用矩阵"
        />

        <div className="grid lg:grid-cols-[300px_1fr] gap-8">
          {/* Left sidebar */}
          <div className="space-y-1.5">
            {AGRICULTURE_APPS.map((item) => {
              ...
              return (
                <motion.button
                  ...
                  whileHover={{ x: 4 }}
                  whileTap={{ scale: 0.98 }}
                  className={`w-full text-left px-5 py-3.5 rounded-xl text-sm font-medium transition-all duration-300 flex items-center gap-3 group ${
                    active
                      ? 'bg-gradient-to-r from-agri-500 to-emerald-600 text-white shadow-lg shadow-agri-200/50'
                ...
                >
                  ...
                  {active && (
                    <HiArrowRight className="w-4 h-4 opacity-70" />
                  )}
                </motion.button>
              )
            })}
                ...
                <div className="p-8">
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    {app.features.map((feat, idx) => (
                      <div
                        key={idx}
                        className="flex items-start gap-3 bg-agri-50/70 rounded-xl p-4 border border-agri-100/50 hover:border-agri-200 hover:bg-agri-50 transition-all duration-200 cursor-default"
                      >
