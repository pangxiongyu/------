...
import { HiAdjustments, HiCheckCircle, HiCloud, HiCursorClick, HiLocationMarker, HiMap, HiMinus, HiPlus, HiRefresh } from 'react-icons/hi';
const workflowsteps = [{
  title: '选择无人机起点',
...
export default function FlightParamsPage() {
  ...
  const update = (key, value) => setlocal(current => ({
    ...current,
    [key]: value
  }));
  ...
  return <PageContainer className="bg-gradient-to-b from-slate-950 via-emerald-950 to-slate-50">
      <div className="mx-auto max-w-7xl px-6 py-14 md:py-18">
        <SectionTitle badge="飞行任务配置" title="先配置参数，再从导航进入三维控制" subtitle="这里用于选择场景、确认风场、设置起点和任务点数量；三维页面只保留顶部导航中的「三维控制」一个入口。" light />
            ...
            duration: 0.45
              ...
              <GlassCard variant="premium" hover={false} className="!rounded-lg !p-6">
                ...
                <div className="space-y-3">
                  {workflowsteps.map((step, index) => <WorkflowStep key={step.title} index={index + 1} title={step.title} text={step.text} />)}
                </div>
              </GlassCard>
            </motion.aside>
            ...
            delay: 0.08
              ...
              <GlassCard variant="premium" hover={false} className="!rounded-lg !p-6">
                ...
                <div className="space-y-5">
                  <CounterControl label="无人机起点" value={local.droneCount} min={1} max={12} onChange={value => update('droneCount', value)} description="对应三维控制页中的绿色起飞点数量" />
                  <CounterControl label="任务目标点" value={local.taskCount} min={1} max={20} onChange={value => update('taskCount', value)} description="对应三维控制页中的红色任务点数量" />
                  ...
                  <label className="flex cursor-pointer items-center justify-between gap-4 rounded-lg border border-slate-100 bg-slate-50 px-4 py-3">
                    ...
                    <input type="checkbox" checked={local.autoAssignment} onChange={event => update('autoAssignment', event.target.checked)} className="h-5 w-5 shrink-0 accent-emerald-500" />
                  </label>
              ...
              <GlassCard variant="premium" hover={false} className="!rounded-lg !p-6">
                ...
                <div className="space-y-3">
                  {selectedscene.windLayers.map(layer => <WindLayerRow key={layer.id} layer={layer} active={layer.id === selectedscene.focusLayerId} />)}
                </div>
              </GlassCard>
...
function SceneFact({
  ...
  return <div className="flex items-center justify-between gap-3 rounded-lg border border-slate-100 bg-slate-50 px-4 py-3">
...
function WorkflowStep({
  ...
  return <div className="flex gap-3 rounded-lg border border-slate-100 bg-slate-50 px-4 py-3">
...
function CounterControl({
  ...
  return <div className="rounded-lg border border-slate-100 bg-white px-4 py-4">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex h-10 shrink-0 items-center overflow-hidden rounded-lg border border-slate-200 bg-slate-50">
          <button type="button" onClick={() => setclampedvalue(value - 1)} className="flex h-10 w-10 items-center justify-center text-slate-500 transition-colors hover:bg-white hover:text-agri-600" aria-label={`减少${label}`}>
            <HiMinus className="h-4 w-4" />
          </button>
          <input type="number" min={min} max={max} value={value} onChange={event => setclampedvalue(Number(event.target.value))} className="h-10 w-14 border-x border-slate-200 bg-white text-center text-base font-black text-slate-900 outline-none" />
          <button type="button" onClick={() => setclampedvalue(value + 1)} className="flex h-10 w-10 items-center justify-center text-slate-500 transition-colors hover:bg-white hover:text-agri-600" aria-label={`增加${label}`}>
...
function WindLayerRow({
  ...
  return <div className={`rounded-lg border px-4 py-3 ${active ? 'border-agri-200 bg-agri-50' : 'border-slate-100 bg-white'}`}>
      <div className="flex items-center justify-between gap-3">
        ...
        {active && <span className="inline-flex items-center gap-1 rounded-full bg-agri-500 px-2.5 py-1 text-xs font-bold text-white">
...
function LayerMetric({
  ...
  return <div className="rounded-md bg-white/80 px-2 py-2">
...
function StatusBadge({
  ...
  return <span className="inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1.5 text-xs font-bold text-agri-700">
      <HiCheckCircle className="h-3.5 w-3.5" />
      {children}
    </span>;
}
