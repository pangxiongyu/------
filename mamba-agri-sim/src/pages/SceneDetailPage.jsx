...
export default function SceneDetailPage() {
  ...
  const metricRows = [
    ['地形结构', selectedScene.terrain],
    ['作物类型', selectedScene.crop],
    ['作业高程', selectedScene.altitude],
    ['风场配置', selectedScene.wind],
  ]
          ...
          >
            <GlassCard variant="premium" hover={false} className="!rounded-lg !p-6">
              ...
              <div className="mt-6 grid gap-3">
                {metricRows.map(([label, value]) => (
                  <div key={label} className="flex items-center justify-between rounded-lg border border-slate-100 bg-slate-50 px-4 py-3">
            ...
            <GlassCard variant="premium" hover={false} className="!rounded-lg !p-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {selectedScene.highlights.map((item) => (
                  <span key={item} className="rounded-lg bg-agri-50 px-3 py-3 text-sm font-bold text-agri-800">
                    {item}
                  </span>
                ))}
              </div>
...
function SceneTopography({ sceneId }) {
  const rows = sceneId === 'modern'
    ? ['M60 260 L340 80', 'M20 160 L380 160', 'M90 310 L360 120', 'M30 230 L330 55']
    : ['M0 290 C70 250 130 265 200 220 C270 175 320 195 400 130', 'M0 235 C80 215 135 225 205 185 C270 150 330 160 400 115', 'M0 180 C70 165 130 175 205 140 C280 100 340 115 400 75']

  return (
    <svg viewBox="0 0 400 320" className="h-full w-full" preserveAspectRatio="none">
      <rect width="400" height="320" fill="rgba(5,150,105,0.18)" />
      {rows.map((d, index) => (
        <path key={d} d={d} fill="none" stroke={index % 2 ? '#a7f3d0' : '#facc15'} strokeWidth="8" strokeLinecap="round" opacity="0.55" />
      ))}
