...
import { SUMMARY_CARDS } from '../../data/chartData'
      ...
      {SUMMARY_CARDS.map((card, idx) => (
        <motion.div key={idx} variants={item}>
          <GlassCard variant="premium" className="relative overflow-hidden">
            ...
            <div className={`absolute top-0 left-4 right-4 h-[2px] rounded-full bg-gradient-to-r ${
              card.up ? 'from-agri-400 to-emerald-300' : 'from-red-400 to-rose-300'
            }`} />

            <div className="flex items-center justify-between mb-3">
              <span className="text-sm text-gray-500 font-medium">{card.title}</span>
              <span className={`flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full ${
                card.up ? 'text-agri-700 bg-agri-50' : 'text-red-600 bg-red-50'
              }`}>
                {card.up
            ...
            <div className="mt-3 flex items-end gap-[2px] h-8 opacity-30">
              {Array.from({ length: 14 }).map((_, i) => (
                <div
                  key={i}
                  className={`flex-1 rounded-sm ${card.up ? 'bg-agri-400' : 'bg-red-400'}`}
                  style={{
